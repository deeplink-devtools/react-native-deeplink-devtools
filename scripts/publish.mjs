...
// failure is safe (same reasoning as ADR-0011).
...
// Publish dependencies before dependents (core before the adapters, the CLI
// last) so an install that races the release never sees a dependent whose
// internal deps are not on the registry yet. Counting in-repo deps is enough
// to order this workspace graph; no full topo sort needed.
